...
from classwork_02_04_05 import MyTimeMath
...
class MyDateTime(MyTimeMath):
    ...
    def __sub__(self, other: MyDateTime) -> MyDateTime:
        ...
        hours_offset = 0
        if new_time.hours < 0:
            hours_offset = abs(new_time.hours) // 24
            new_time.hours = abs(new_time.hours) % 24

        day = self.day - other.day - hours_offset
        days_offset = 0
        if day < 0:
            days_offset = abs(day) // 30
            day = abs(day) % 30

        month = self.month - other.month - days_offset
        month_offset = 0
        if month < 0:
            month_offset = abs(month) // 12
            month = abs(month) % 12
